...
#' @param pct_group This option is used to determine which variable of the row and column
#' variables should add up to 100 %. Multiple variables can be specified in a vector to
#' generate multiple group percentages. You can also use the keywords "row_pct" or "col_pct"
#' to calculate total percentages for rows and columns regardless of the respective other dimension.
...
any_table <- function(data_frame,
    ...
    row_expand <- expand_brackets(rows)

    if (row_expand[[2]]){
        rows <- names(row_expand[[1]])
    }
    else{
        row_expand <- NULL
    }
    ...
    # Expand brackets first to be able to identify the unique variables used afterwards
    column_expand <- expand_brackets(columns)

    if (column_expand[[2]]){
        columns <- names(column_expand[[1]])
    }
    else{
        column_expand <- NULL
    }
    ...
    if (!pre_summed){
        ...
        valid_stats   <- statistics[statistics %in% c(names(list_of_statistics), "sum_wgt", "pct_value",
                                                      "pct_group", "pct_total", "pct_block", paste0("p", 1:100))]
    ...
    # If pct_block is selected, make sure sum is also part of statistics
    flag_remove_block_sum <- FALSE

    pct_block <- get_origin_as_char(pct_block, substitute(pct_block))

    if (!all(pct_block %in% c("rows", "columns"))){
        print_message("WARNING", c("Invalid option for <pct_block>. <pct_block> can be 'rows' or 'columns', 'rows' will be used."))
        pct_block  <- "rows"
    }

    if (!"sum" %in% statistics){
        if ("pct_block" %in% tolower(statistics)){
            statistics            <- c(statistics, "sum")
            flag_remove_block_sum <- TRUE
        }
    }
    else{
        if ("pct_block" %in% tolower(statistics) && flag_remove_sum){
            flag_remove_block_sum <- TRUE
        }
    }
    ...
    if (flag_remove_sum && !flag_remove_block_sum){
    ...
    for (row_combi in rows){
        ...
        for (col_combi in columns){
            ...
            # Make a copy of the value variables, because the vector is altered
            # during the block percentage calculation.
            pivot_values <- value_vars

            if (length(pct_block) > 0){
                #-------------------------------------------------------------------------#
                monitor_df <- monitor_df |> monitor_next("Block percentages", "Block percentages")
                #-------------------------------------------------------------------------#

                # Loop over rows and columns. The calculation is basically the same, but the
                # group variable selection is different.
                for (block in pct_block){
                    # On rows: select all  row variables but the last and all column variables
                    if (block == "rows"){
                        block_group  <- c(id_vars[-length(id_vars)], col_combi_vars)
                    }
                    # On columns: select all row variables and all column variables but the last
                    else{
                        block_group  <- c(id_vars, col_combi_vars[-length(col_combi_vars)])
                    }

                    # Get the sum variables and merge back the group maximums to the data frame
                    block_values <- pivot_values[endsWith(pivot_values, "_sum")]

                    combi_df <- suppressMessages(combi_df |>
                         summarise_plus(class      = block_group,
                                        values     = block_values,
                                        statistics = "max",
                                        notes      = FALSE,
                                        merge_back = TRUE))
                ...
                # Remove sum variables if they weren't specified in the statistics.
                # Also remove them from pivot vector.
                if (flag_remove_block_sum){
    ...
    if (length(row_expand) == 0){
        any_tab <- data.table::rbindlist(part_combi_list, fill = TRUE, use.names = TRUE)
    }
    # If there are expanded brackets in the row combinations, also insert TYPE variable
    # back in to be able to sort the results.
    else{
        any_tab <- data.table::rbindlist(
            lapply(names(part_combi_list), function(combination){
                # Insert TYPE variable back in
                data_part           <- part_combi_list[[combination]]
                data_part[["TYPE"]] <- combination
    ...
    # Sort row header columns if there are expanded combinations. What this does is
    # putting the combinations together in a way, so that what belongs together is
    # printed together. Meaning same combination roots are put below each other and
    # their cells get merged in the Excel table later.
    if (length(row_expand) > 0){
        # Sorting order: original not expanded group, all row header variables but the last
        #                expanded groups + last row header variable
        # This order keeps the root combinations together but the last variables
        # apart from another.
        sort_vars <- c("TYPE_ORIG", ordered_cols[-length(ordered_cols)],
                       "TYPE",      ordered_cols[length(ordered_cols)])

        # Get the unique values per variable in order of appearance. This is crucial
        # to keep the base order as is.
        levels_list <- lapply(any_tab[sort_vars], function(variable){
    ...
    # Remove suppressed format expressions per do over for all row header variables
    # and drop column variables accordingly.
    any_tab <- suppressMessages(any_tab |>
                        if.(ordered_cols != "!:") |>
                        dropp(":_!:"))
    any_header <- suppressMessages(any_header |> dropp(":_!:"))
...
set_col_variable_labels <- function(column_header, var_labels){
    ...
    # Order list by the length of the variable names (descending) to prevent that, if
    # the shortest variable name is at the top, and other variables follow with the
    # same base name (like: age, age1, age2), it would overwrite all other labels for
    # the following variables with the same base name.
    var_labels <- var_labels[order(nchar(names(var_labels)), decreasing = TRUE)]
...
build_multi_header <- function(var_names,
    ...
    if (length(var_labels) > 0 && any(startsWith(tolower(names(var_labels)), "block"))){
        ...
        root_names    <- sub("^([^_]+_[^_]+)_.*$", "\\1", var_names[start_var:length(var_names)])
        ...
        # overwrite names of first table
        block_labels <- stats::setNames(block_labels, names(merged_header))

        # now rbind
        merged_header <- data.table::rbindlist(list(block_labels, merged_header), use.names = TRUE)
    }
...
# Helper functions
###############################################################################
#' Make Space Between Plus Symbols Equal
#'
#' @description
#' Normalizes the space between plus symbols.
#'
#' @param combination Combination of variables connected with plus symbols.
#'
#' @return
#' Returns a normalized variable combination.
#'
#' @noRd
normalize_combination <- function(combination){
    # Remove extra spaces
    combination <- gsub("\\s*\\+\\s*", " + ", combination)
    combination <- gsub("\\s+", " ", combination)
    trimws(combination)
}


#' Expand Brackets To Generate Multiple Combinations
#'
#' @description
#' Expands variables passed in brackets to multiple full combinations.
#' "state + sex + (year age)" would become "state + sex + year" and
#' "state + sex + age".
#'
#' Can also expand combinations like "(state sex) + (year age)".
#'
#' @param combination Combination of variables connected with plus symbols.
#'
#' @return
#' Returns expanded variable combinations.
#'
#' @noRd
expand_combination <- function(combination){
    combination <- normalize_combination(combination)

    # Return combination as is, if there is no bracket to expand
    if (!grepl("\\(", combination)){
        return(stats::setNames(list(combination), combination))
    }

    # Find bracket groups
    matches <- gregexpr("\\([^\\)]+\\)", combination)
    groups  <- regmatches(combination, matches)[[1]]
    ...
    combinations <- expand.grid(variable_list, stringsAsFactors = FALSE)

    # Create combination template to insert new combinations into
    template <- combination

    for (i in seq_along(groups)){
        template <- sub("\\([^\\)]+\\)", "%s", template)
    }

    # Put together the final expanded variable combinations
    results <- apply(combinations, 1, function(row){
        normalize_combination(do.call(sprintf, c(template, as.list(row))))
    })

    # mapping
    stats::setNames(as.list(rep(combination, length(results))), results)
}
...
#' "state + sex + age"
#'
#' @param combinations Combination vector of variables connected with plus symbols.
#'
#' @return
#' Returns expanded variable combination list.
#'
#' @noRd
expand_brackets <- function(combinations){
    # Expand all combinations
    result <- unlist(lapply(combinations, expand_combination), recursive = FALSE)

    # Remove duplicate combinations
    names <- result[collapse::funique(names(result))]
    list(names, any(grepl("\\(", unlist(names))))
}
